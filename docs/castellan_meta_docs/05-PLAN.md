...
- `workspace/load.ts` — tiered file loading per `07-SPEC-WORKSPACE.md`; **strips frontmatter and HTML comments before injection**; enforces per-file and total budgets with a named failure
- `model/prompt-style.ts` — render authored markdown per `capabilities.promptStyle` (delimiters, intensity)
...
- [ ] Same `AGENT.md` renders with XML delimiters under `delimiters: xml` and plain sections under `plain`
...
- [ ] Eval matrix settles the two open `promptStyle` questions, results in `evals/prompt-style/`:
      (a) `examplesIn: system` vs `user` — Anthropic and OpenAI give opposite guidance
      (b) `intensity: emphatic` vs `neutral` on the smallest model — confirms the inversion is real
- [ ] `castellan eval rules` implemented: measures `perRuleSuccess` with a verifiable-instruction probe so the rule guard validates something real rather than a guessed constant
...
- `castellan workspace validate` — frontmatter, budgets, tier order, rule count vs reliability target, example count and diversity, prose/structure match against bound channel capabilities
...
- [ ] `castellan workspace validate` flags a bulleted `AGENT.md` when every bound channel is `markdown: none|basic`, and flags a rule with no rationale clause
...
- `context/compaction-notice.ts` — runtime-generated line telling the model context compacts automatically, injected when compaction is enabled
...
- [ ] With `compactionNotice: true`, a long session does not show the model wrapping up work early on budget grounds; with it false, the behaviour reappears
