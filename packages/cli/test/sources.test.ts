...
function run(action: string, rest: readonly string[] = [], git?: Git): number {
...
describe("the registry", () => {
    test("with no file at all, the built-ins are what you get", () => {
    ...
    test("built-ins are never written to the file, so a moved repo is fixed by an upgrade", () => {
    ...
    test("a user entry overrides a built-in and keeps its position", () => {
    ...
    test("a built-in can be removed, and stays removed", () => {
        const first = DEFAULT_SOURCES[0]?.name as string
    ...
    test("adding a removed built-in back re-enables it", () => {
        const first = DEFAULT_SOURCES[0]?.name as string
    ...
    test("removing something that was never there is reported, not invented", () => {
        expect(removeSource("nothing-like-this", env)).toBe(false)
    })

    test("unreadable JSON fails loudly rather than falling back to the built-ins", () => {
        writeFileSync(join(home, "sources.json"), "{ not json")
        expect(() => loadSources(env)).toThrow()
    })

    test("a name that could not be a directory is refused", () => {
...
describe("what a person types for a repository", () => {
    ...
    test("a clone URL, with .git trimmed", () => {
        expect(parseSourceUrl("https://github.com/a/b.git").url).toBe("https://github.com/a/b")
    })

    test("the page URL from the address bar carries the branch and the subdirectory", () => {
...
describe("a skill reference", () => {
    test("bare, and source-qualified", () => {
        expect(parseSkillRef("pdf")).toEqual({ skill: "pdf" })
        expect(parseSkillRef("anthropic/pdf")).toEqual({ source: "anthropic", skill: "pdf" })
    })

    test("three segments is neither, and says so", () => {
        expect(() => parseSkillRef("a/b/c")).toThrow()
    })

    test("a path is recognised before it is resolved, so a typo reads as a missing path", () => {
...
describe("fetching", () => {
    ...
    test("a clone lands in the cache, and the meta records commit and count", () => {
        only()
        const { git } = fakeGit(SKILLS)
        expect(run("update", [], git)).toBe(0)
    ...
    test("a nested skill is found, not swallowed by its parent", () => {
        only()
        const { git } = fakeGit(SKILLS)
        run("update", [], git)
    ...
    test("the partial-clone filter is tried, and its refusal is a retry rather than a failure", () => {
        only()
    ...
    test("a failed fetch leaves the previous copy in place and exits non-zero", () => {
        only()
        run("update", [], fakeGit(SKILLS).git)
        const before = readCatalogue(
            { name: "test", url: "https://example.test/skills", path: "skills" },
            env,
        ).length
        expect(run("update", [], fakeGit([], { failClone: true }).git)).toBe(1)
    ...
    test("git's own stderr survives into the message", () => {
        only()
        run("update", [], fakeGit([], { failClone: true }).git)
        expect(written.includes("repository not found")).toBe(true)
    })

    test("updating a name that is not configured names what is", () => {
        expect(run("update", ["nope"], fakeGit(SKILLS).git)).toBe(1)
        expect(written.includes("no source called nope")).toBe(true)
    })

    test("a skill whose frontmatter will not load is listed with its problem, not dropped", () => {
        only()
...
describe("search", () => {
    ...
    test("the right skill ranks first", () => {
        seeded()
        expect(run("search", ["pull", "the", "tables", "out", "of", "this", "pdf"])).toBe(0)
    ...
    test("a query about nothing in the catalogue matches nothing, and exits 0", () => {
        seeded()
        // The criterion that catches a scorer which always returns something. Not a failure: asking is
        // allowed, and the answer is "no".
        expect(run("search", ["who", "won", "the", "1998", "world", "cup"])).toBe(0)
        expect(written.includes("matches")).toBe(true)
    })

    test("no query lists everything, which is how you browse a source", () => {
        seeded()
        expect(run("search", [])).toBe(0)
        expect(written.includes("4 skills")).toBe(true)
    })

    test("runnable files are counted in the listing, because that is the trust signal", () => {
        seeded()
        run("search", ["pdf"])
        expect(written.includes("1 runnable file")).toBe(true)
    })

    test("a cold source is fetched on first search, and says so", () => {
        for (const spec of DEFAULT_SOURCES) removeSource(spec.name, env)
        addSource({ name: "test", url: "https://example.test/skills", path: "skills" }, env)
        expect(run("search", ["pdf"], fakeGit(SKILLS).git)).toBe(0)
        expect(written.includes("for the first time")).toBe(true)
    })

    test("with every source unfetchable, search fails rather than reporting an empty catalogue", () => {
        for (const spec of DEFAULT_SOURCES) removeSource(spec.name, env)
        addSource({ name: "test", url: "https://example.test/skills", path: "skills" }, env)
        expect(run("search", ["pdf"], fakeGit([], { failClone: true }).git)).toBe(1)
    })

    test("with no sources at all, it says that instead of searching nothing", () => {
        for (const spec of DEFAULT_SOURCES) removeSource(spec.name, env)
        expect(run("search", ["pdf"])).toBe(1)
        expect(written.includes("no sources configured")).toBe(true)
    })
})

describe("add and list through the command", () => {
    test("a URL with no name is named after the owner", () => {
        expect(run("add", ["https://github.com/openclaw/skills"])).toBe(0)
        expect(loadSources(env).some((spec) => spec.name === "openclaw")).toBe(true)
    })

    test("a derived name that collides refuses rather than overwriting", () => {
        run("add", ["https://github.com/openclaw/skills"])
        written = ""
        expect(run("add", ["https://github.com/openclaw/other-skills"])).toBe(1)
        expect(written.includes("already a source")).toBe(true)
    })

    test("an explicit name may replace an existing source", () => {
        run("add", ["mine", "https://example.test/one"])
        expect(run("add", ["mine", "https://example.test/two"])).toBe(0)
        expect(loadSources(env).find((spec) => spec.name === "mine")?.url).toBe(
            "https://example.test/two",
        )
    })

    test("--path beats the path inside a page URL", () => {
        expect(
            sourcesCommand({
                action: "add",
    ...
    test("list reports a source that has never been fetched as exactly that", () => {
        expect(run("list")).toBe(0)
        expect(written.includes("never fetched")).toBe(true)
    })

    test("--json carries the registry path, so a script does not guess it", () => {
        expect(sourcesCommand({ action: "list", rest: [], json: true, env })).toBe(0)
    ...
    test("every built-in removed is reported, not rendered as an empty table", () => {
        for (const spec of DEFAULT_SOURCES) removeSource(spec.name, env)
        written = ""
        expect(run("list")).toBe(0)
...
describe("init --skills find", () => {
    ...
    function find(words: string | undefined, git?: Git): string {
    ...
    test("the best match is installed", () => {
        const manifestPath = find("pull the tables out of this pdf", fakeGit(SKILLS).git)
        expect(written.includes("test/pdf")).toBe(true)
        expect(existsSync(join(dirname(manifestPath), "skills", "pdf", "SKILL.md"))).toBe(true)
    })

    test("the runners-up are named, so the pick is inspectable rather than magic", () => {
        ...
        find("scale the node up and add memory", fakeGit(SKILLS).git)
        expect(written.includes("also matched")).toBe(true)
    })

    test("provenance is written, so `skills list` can say where it came from", () => {
        const manifestPath = find("pull the tables out of this pdf", fakeGit(SKILLS).git)
    ...
    test("an unreachable source leaves a valid agent and says why", () => {
        // The rule this path exists under: init must not fail because a network did.
        const manifestPath = find("pdf tables", fakeGit([], { failClone: true }).git)
    ...
    test("a phrase that matches nothing installs nothing, and does not substitute something else", () => {
        find("who won the 1998 world cup", fakeGit(SKILLS).git)
        expect(written.includes("matches those words")).toBe(true)
        expect(written.includes("installed")).toBe(true)
    })

    test("no words at all is reported rather than silently skipped", () => {
    ...
    test("nothing happens at all unless the answer was `find`", () => {
        const manifestPath = agentFor()
        written = ""
        findAndInstallSkill({
